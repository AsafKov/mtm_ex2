...
ostream& operator<<(ostream &os, const ExamDetails &exam) {
    return os << "Course Number: " << exam.courseId << std::endl << "Time: " << exam.examDay << "." << exam.examMonth << " at "
        << (int)exam.examHour << ":" << ((exam.examHour - (int)exam.examHour <= 1e-7) ? "00" : "30") << std::endl << "Duration: "
